...
def create_app(test_config=None):
  ...
  def getQuestions():
      ...
      categories = list(map(Category.format, Category.query.all()))
      questions = list(map(Question.format, questions_page.items))
  ...
  def add_question():
      if request.data:
          new_question_raw = json.loads(request.data.decode('utf-8'))
          if ('question' in new_question_raw
                and 'answer' in new_question_raw
                and 'difficulty' in new_question_raw
                and 'category' in new_question_raw):
              new_question = Question(
                  question=new_question_raw['question'],
                  answer=new_question_raw['answer'],
                  difficulty=new_question_raw['difficulty'],
                  category=new_question_raw['category']
              )
